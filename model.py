...
class denoiser(object):
    ...
    def evaluate(self, iter_num, test_data, sample_dir, summary_merged, summary_writer):
        ...
        for idx in xrange(len(test_data)):
            ...
            noisy_image = np.copy(test_data[idx])
            ...
            output_clean_image, noisy_image, psnr_summary = self.sess.run(
                ...
                feed_dict={self.Y_: clean_image, self.X: noisy_image,
    ...
    def train(self, data, eval_data, batch_size, ckpt_dir, epoch, lr, sample_dir, eval_every_epoch=2):
        ...
        for epoch in xrange(start_epoch, epoch):
            ...
            for batch_id in xrange(start_step, numBatch):
                ...
                noisy_images = np.copy(data[batch_id * batch_size:(batch_id + 1) * batch_size, :, :, :])
                #print(batch_images.shape) # (128, 40, 40, c_dim)
                for tmp in range (0,128):
                    tmp_array = noisy_images[tmp,:,:,:]
                    tmp_array = np.squeeze(tmp_array,2)
                    ...
                    jpg_quality40_array = jpg_quality40_array.reshape(40,40,1)
                    noisy_images[tmp,:,:,:] = jpg_quality40_array
                ...
                _, loss, summary = self.sess.run([self.train_op, self.loss, merged],
                                                 feed_dict={self.Y_: batch_images, self.lr: lr[epoch],
                                                            self.is_training: True, self.X: noisy_images})
    ...
    def test(self, test_files, ckpt_dir, save_dir):
        ...
        for idx in xrange(len(test_files)):
            ...
            noisy_image = np.copy(load_images(test_files[idx]))
            noisy_image_h = np.size(noisy_image, 1)
            noisy_image_w = np.size(noisy_image, 2)
            noisy_image = np.squeeze(noisy_image, 0)
            noisy_image = np.squeeze(noisy_image, 2)
